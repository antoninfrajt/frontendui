 ...
 * This component renders a button that opens a dialog for confirming the addition or removal of a guarantor.
 * It uses async actions for insertion or deletion, displays loading and error * states, and calls `onDone` after completion. 
 *
 * @component
 * @param {Object} props - The properties for the GuarrantCUDButton component.
 ...
 */
 export const GuarrantorButton = ({ operation, guarant, onDone = (guarant) => {}, children, ...props }) => { // Configuration for each operation type
    ...
    const { error, loading, fetch } = useAsyncAction(asyncAction, guarant, { deferred: true }); // Handle button click: perform async action and call onDone
    const handleClick = async () => {
        try {
            const result = await fetch(guarant);
            onDone(result);
        }
        catch (error) { // Error is handled by ErrorHandler
    ...
    return (
        <ButtonWithDialog
            ...
            onClick={handleClick}
        >
        {/*Show error message or loading state inside diaglog*/}
